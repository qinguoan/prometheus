...
func (s *memorySeries) resetChunks(offset int) {
	s.chunkDescs = s.chunkDescs[s.persistWatermark:]
	s.chunkDescsOffset = offset
...
func (s *memorySeries) preloadChunks(
	...
	iter := &boundedIterator{
		...
		start: pinnedChunkDescs[0].FirstTime().Add(-1),
	}
...
func (s *memorySeries) preloadChunksForInstant(
	...
	if !through.Before(lastSample.Timestamp) &&
		...
		iter := &boundedIterator{
			...
			start: lastSample.Timestamp.Add(-1),
		}
		return iter, nil
	}
...
func (s *memorySeries) preloadChunksForRange(
	...
	if s.chunkDescsOffset != 0 && from.Before(firstChunkDescTime) {
		...
		//log.Infof("fp: %v, watermark: %v chunkDescOffset: %v")
...
// ValueAtOrBeforeTime implements SeriesIterator.
func (it *memorySeriesIterator) ValueAtOrAfterTime(t model.Time) model.SamplePair {
	// The most common case. We are iterating through a chunk.
	if it.chunkIt != nil {
		containsT, err := it.chunkIt.Contains(t)
		if err != nil {
...
// ValueAtTime implements SeriesIterator.
func (it *singleSampleSeriesIterator) ValueAtOrAfterTime(t model.Time) model.SamplePair {
	if it.samplePair.Timestamp.Before(t) {
		return model.ZeroSamplePair
	}
	return it.samplePair
}
...
// ValueAtTime implements SeriesIterator.
func (i nopSeriesIterator) ValueAtOrAfterTime(t model.Time) model.SamplePair {
	return model.ZeroSamplePair
}
