...
import (
	...
	"github.com/qinguoan/prometheus/storage/local/index"
	"github.com/qinguoan/prometheus/util/flock"
)
...
func (p *persistence) persistChunks(fp model.Fingerprint, chunks []chunk.Chunk, idx int) (index int, err error) {
	f, err := p.openChunkFileForWriting(fp, idx)
	if err != nil {
		return -1, err
	}
...
func (p *persistence) persistChunksAtPreviousIndex(fp model.Fingerprint, chunks []chunk.Chunk, idx int, ok bool) (index int, err error) {
	f, err := p.openChunkFileForWriting(fp, idx)
	if err != nil {
		return -1, err
	}
	defer p.closeChunkFile(f)
	if ok {
		if _, err = f.Seek(-chunkLenWithHeader, os.SEEK_END); err != nil {
			return -1, err
		}
	} else {
		if _, err = f.Seek(0, os.SEEK_END); err != nil {
...
func (p *persistence) loadChunks(fp model.Fingerprint, indexes []int, indexOffset int) ([]chunk.Chunk, error) {
	// for we will change value in indexes, so clone it first.
	cloneIndexes := make([]int, len(indexes))
	copy(cloneIndexes, indexes)

	chunks := make([]chunk.Chunk, 0, len(cloneIndexes))
	for i := 0; i < len(cloneIndexes); i++ {
		cloneIndexes[i] += indexOffset
	}

	for idx := len(DefaultRetentionList) - 1; idx >= 0; idx-- {
		idxIndexes := make([]int, 0)

		if len(cloneIndexes) == 0 {
			return chunks, nil
		}

		if cap := p.fileChunkCapacity(fp, idx); cap > 0 {

			n := sort.Search(len(cloneIndexes), func(i int) bool {
				return cloneIndexes[i] >= cap
			})

			switch n {
			case 0:
				idxIndexes = nil
			case len(cloneIndexes):
				idxIndexes = cloneIndexes
				cloneIndexes = nil
			default:
				idxIndexes = cloneIndexes[:n]
				cloneIndexes = cloneIndexes[n:]
			}
			for i := range cloneIndexes {
				cloneIndexes[i] -= cap
			}
			// log.Infof("fp: %v, idx: %v, loadindex: %v, left: %v, cap: %v",
			// 	fp, idx, idxIndexes, cloneIndexes, cap)
		}
		...
		f, err := p.openChunkFileForReading(fp, idx)
		if err != nil {
			return nil, err
		}
		defer f.Close()
		...
		for i := 0; i < len(idxIndexes); i++ {
			// This loads chunks in batches. A batch is a streak of
			// consecutive chunks, read from disk in one go.
			batchSize := 1
			if _, err := f.Seek(offsetForChunkIndex(idxIndexes[i]), os.SEEK_SET); err != nil {
				return nil, err
			}

			for ; batchSize < chunkMaxBatchSize &&
				i+1 < len(idxIndexes) &&
				idxIndexes[i]+1 == idxIndexes[i+1]; i, batchSize = i+1, batchSize+1 {
			}
...
func (p *persistence) fileChunkCapacity(fp model.Fingerprint, idx int) int {
	f := p.fileNameForFingerprint(fp, idx)
	fi, err := os.Stat(f)
	if err != nil {
		return 0
	}
	return int(fi.Size()) / chunkLenWithHeader
}

func (p *persistence) getLatestChunk(fp model.Fingerprint, idx int) (chunk.Chunk, error) {
	f, err := p.openChunkFileForReading(fp, idx)
	if err != nil {
		return nil, err
	}
	defer f.Close()
	_, err = f.Seek(-chunkLenWithHeader, os.SEEK_END)
	if err != nil {
		return nil, err
	}
...
func (p *persistence) loadChunkDesc(fp model.Fingerprint, idx int) ([]*chunk.Desc, error) {
	f, err := p.openChunkFileForReading(fp, idx)
...
func (p *persistence) loadChunkDescs(fp model.Fingerprint, offsetFromEnd int) ([]*chunk.Desc, error) {
	total := make([]*chunk.Desc, 0)
	for idx := len(DefaultRetentionList) - 1; idx >= 0; idx-- {
		cds, err := p.loadChunkDesc(fp, idx)
		if err != nil {
			return nil, err
		}
		total = append(total, cds...)
	}
	return total[:len(total)-offsetFromEnd], nil
}
	...
	for m := range iter {
		func() { // Wrapped in function to use defer for unlocking the fp.
			...
			if m.series.chunkDescsOffset < 0 && m.series.persistWatermark > 0 {
				log.Infof("fp: %v will cause panic chunkDescsOffset: %v persistWatermark: %v", m.fp, m.series.chunkDescsOffset, m.series.persistWatermark)
...
func (p *persistence) isNeedRetetion(fp model.Fingerprint, idx, totalChunks int) bool {
	retention := DefaultRetentionList[idx]
	// file := p.fileNameForFingerprint(fp, idx)
	// fi, err := os.Stat(file)
	// if err != nil {
	// 	return false, err
	// }
	// if fi.Size() >= retention.LimitedBytes() {
	// 	return true, nil
	// }
	if totalChunks > int(retention.ReservedChunks()) {
		return true
	}
...
// retrun left chunks number, reduced chunk number and error if any.
func (p *persistence) doRetention(fp model.Fingerprint, idx int) (totalchunks, reduced int, err error) {
	f, err := p.openChunkFileForReading(fp, idx)
	...
	totalchunks = int(fi.Size()) / chunkLenWithHeader

	reservedCount := int(DefaultRetentionList[idx].ReservedChunks())

	if totalchunks <= reservedCount || fi.Size() < DefaultRetentionList[idx].LimitedBytes() {
		return
	}
	...
	next := nextInterval(idx)
	var header *chunk.Desc
	nextCapacity := p.fileChunkCapacity(fp, idx+1) > 0
	if nextCapacity {
		c, err := p.getLatestChunk(fp, idx+1)
		if err != nil {
			return totalchunks, reduced, err
		}
		header = chunk.NewDesc(c, c.FirstTime())
		firstTime, err = c.NewIterator().LastTimestamp()
		if err != nil {
			log.Infof("next idx: %v has chunks but get lasttime err: %v", idx+1, err)
			return totalchunks, reduced, err
		}
		firstTime = firstTime.Add(next)
	} else {
		header = chunk.NewDesc(chunk.New(), firstTime)
	}
	...
	end := totalchunks - reservedCount
	var found int
	for chunkIndex := 0; chunkIndex < end; chunkIndex++ {
		// This loads chunks in batches. A batch is a streak of
		// consecutive chunks, read from disk in one go.
		batchSize := 1
		if _, err = f.Seek(offsetForChunkIndex(chunkIndex), os.SEEK_SET); err != nil {
			return totalchunks, reduced, err
		}

		for ; batchSize < chunkMaxBatchSize &&
			chunkIndex+1 < end; chunkIndex, batchSize = chunkIndex+1, batchSize+1 {
		}
		readSize := batchSize * chunkLenWithHeader
		if cap(buf) < readSize {
			buf = make([]byte, readSize)
	...
	if found > 0 {
		chunks = append(chunks, header.C)
	}

	if n := len(chunks); n > 0 {

		if _, err = p.persistChunksAtPreviousIndex(fp, chunks, idx+1, nextCapacity); err != nil {
			return
		}
		atomic.AddInt64(&chunk.NumMemChunks, -int64(n))
		chunk.NumMemDescs.Sub(float64(n))
	}
	if _, err = f.Seek(offsetForChunkIndex(totalchunks-reservedCount), os.SEEK_SET); err != nil {
		return
	}

	temp, err := os.OpenFile(p.tempFileNameForFingerprint(fp, idx), os.O_WRONLY|os.O_CREATE, 0640)
	if err != nil {
		return
	}
	defer func() {
		...
		if err == nil {
			err = os.Rename(p.tempFileNameForFingerprint(fp, idx), p.fileNameForFingerprint(fp, idx))
		}
	}()
	...
	reduced = totalchunks - reservedCount - len(chunks)
	if nextCapacity {
		reduced++
	}

	return reservedCount, reduced, nil
}

func (p *persistence) retentionAndPersistChunks(
	fp model.Fingerprint, chunks []chunk.Chunk,
) (
	offset int,
	numReduced int,
	err error,
	...
	if c := len(chunks); c > 0 {
		if _, err = p.persistChunks(fp, chunks, 0); err != nil {
			log.Infof("persist chunks err: %v", err)
			return
		}
	}
	...
	var reduced int
	var index int
	for idx := 0; idx < len(DefaultRetentionList)-1; idx++ {
		index, reduced, err = p.doRetention(fp, idx)
		if err != nil {
			return
		}
	...
	offset += p.fileChunkCapacity(fp, len(DefaultRetentionList)-1)
	return
}
...
func (p *persistence) deleteSeriesFile(fp model.Fingerprint) (int, error) {
	fname := p.fileNameForFingerprint(fp, 0)
...
func (p *persistence) quarantineSeriesFile(fp model.Fingerprint, quarantineReason error, metric model.Metric) error {
	var (
		oldName     = p.fileNameForFingerprint(fp, 0)
...
func (p *persistence) seriesFileModTime(fp model.Fingerprint) time.Time {
	...
	if fi, err := os.Stat(p.fileNameForFingerprint(fp, 0)); err == nil {
		return fi.ModTime()
	}
	return modTime
}
...
func (p *persistence) openChunkFileForWriting(fp model.Fingerprint, retention int) (*os.File, error) {
	...
	return os.OpenFile(p.fileNameForFingerprint(fp, retention), os.O_WRONLY|os.O_CREATE, 0640)
...
func (p *persistence) openChunkFileForReading(fp model.Fingerprint, retention int) (*os.File, error) {
	return os.Open(p.fileNameForFingerprint(fp, retention))
}
	...
	for batchSize := chunkMaxBatchSize; len(chunks) > 0; chunks = chunks[batchSize:] {
		...
		for i, chunk := range chunks[:batchSize] {
			...
			if err := writeChunkHeader(b[i*chunkLenWithHeader:], chunk); err != nil {
				log.Infof("%d chunk header is nil err: %v", err)
				return err
			}
