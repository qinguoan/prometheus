...
// ValueAtOrAfterTime implements the SeriesIterator interface.
func (bit *boundedIterator) ValueAtOrAfterTime(ts model.Time) model.SamplePair {
	if ts < bit.start {
		return model.ZeroSamplePair
	}
	return bit.it.ValueAtOrAfterTime(ts)
}
...
func (s *MemorySeriesStorage) maintainMemorySeries(fp model.Fingerprint) (becameDirty bool) {
	...
	if s.writeMemorySeries(fp, series) {
...
//
// The caller must have locked the fp.
func (s *MemorySeriesStorage) writeMemorySeries(fp model.Fingerprint, series *memorySeries) bool {
	...
	defer func() {
		...
		if cdsCount > 0 {
			...
			s.incNumChunksToPersist(-cdsCount)
			chunk.Ops.WithLabelValues(chunk.PersistAndUnpin).Add(float64(cdsCount))
	...
	chunks := make([]chunk.Chunk, cdsCount)
	for i, cd := range cds {
		chunks[i] = cd.C
	}
	offset, numDropped, persistErr := s.persistence.retentionAndPersistChunks(fp, chunks)
	//log.Infof("fp: %v, total offset on disk: %v, dropped: %v Descoffset: %v", fp, offset, numDropped, series.chunkDescsOffset)
	if persistErr != nil {
		return true
	}
	if numDropped > 0 {
		series.resetChunks(offset)
		...
		series.savedFirstTime = cds[0].FirstTime()
		series.chunkDescsOffset = offset - cdsCount
...
// func (s *MemorySeriesStorage) maintainArchivedSeries(fp model.Fingerprint) {
// 	defer func(begin time.Time) {
...
// 	_, _, err := s.persistence.retentionAndPersistChunks(fp, nil)
// 	if err != nil {
// 		log.Error("Error retention persisted chunks: ", err)
// 	}
